...
md"""# Finding and Adding `neighbor` data points

Here we document methods that connect grid cells at the eddge of a grid subdomain to it nearest neighbors, which can be in different subdomains. 

This currently includes two main sets of methods:

- finding neighbors (`i,j,f`) across subdomains (`neighbor_locations`, `update_location!`)
- adding neighbors as extra rows and columns at the edges of each subdomain (`exchange`)
...
update_location! 			#gets AbstractArray{T,1} and 𝑃, returns AbstractArray{T,1}
									...
									#returns location (x,y,f) arrays
> update_location_cs! 		#uses 𝑃.RelocFunctions / RelocationFunctions_cs, returns AbstractArray{T,1}
> update_location_llc! 		#uses RelocationFunctions_cs
> update_location_PeriodicDomain! 	#uses NeighborTileIndices_PeriodicDomain

NeighborTileIndices_cs 		#gets grid NamedTuple, returns merged NamedTuple
> RelocationFunctions_cs 	#gets MeshArray, returns Array of Function (6,6)
NeighborTileIndices_PeriodicDomain 	#gets MeshArray, returns Array of indices (ni*nj,4)
```
...
Let's load various grids in memory and then select one to demo methods.
...
# ╔═╡ 7bb71c29-67a0-456e-85cc-37376bfcdfff
md"""#### More Grids to Try"""
...
# ╔═╡ b4999ff3-f30f-4533-91ed-31ccd1ce58ba
md"""### Select Grid"""
...
let
	...
	u=[-1.0;20.0;1.0]; uu=deepcopy(u)
	MeshArrays.update_location!(u,Γ)
	[uu,u]
end

# ╔═╡ 0c28fb51-0dc7-4f8a-954a-bf1f877d5e83
md"""### Grid Line Example"""
...
manifest_format = "2.0"
project_hash = "82d145af8c12251ee6660e593113d2f578ff799f"
...
git-tree-sha1 = "13f66493ac4555d54f5e1e32ab1a1a08e483f790"
