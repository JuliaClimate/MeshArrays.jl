...
The chosen example proceeds as follows:
...
Below, this example is repeated for three different grid configurations commonly used in numerical models (see [Grids](https://juliaclimate.github.io/MeshArrays.jl/dev/start/#Grids)). 
"""
...
begin	
	...
	hm1=Mkie.heatmap!(ax1,x,y,γ.write(zin),clims=(-0.25,0.25),tickfont = (4, :black))
	ax2 = Mkie.Axis(fig[1,2])
	hm2=Mkie.heatmap!(ax2,x,y,γ.write(zout),clims=(-0.25,0.25),tickfont = (4, :black))
...
In this cell we instead use a grid that has 6 subdomains, 32x32 points each, covering the six faces of a cube. We note that this _cube sphere_ topology involves connections between subdomain that are slightly more complicated than in the first example. And we call `myDemos.demo2()` which combines steps 2 and 3. 
"""
