

## GridSpec function with category argument:

"""
    GridSpec_MITgcm(category="PeriodicDomain", 
            path=tempname(); np=nothing, ID=:unknown)

- Select one of the pre-defined grids 
    - or by `category` parameter
    - either by `ID` keyword
- Return the corresponding `gcmgrid`
    - incl. path to grid files (`path`).

Input files for these fully supported grids get 
downloaded internally via `MeshArrays.Dataset` if needed.

1. selection by `ID`

- `:onedegree`
- `:LLC90`
- `:LLC270`
- `:CS32`
- `:default`

Example:

```
using MeshArrays
g = GridSpec(ID=:LLC90)
```

2. by `category` and `path`

- `"PeriodicDomain"` (default)
- `"PeriodicChannel"`
- `"CubeSphere"`
- `"LatLonCap"``

Examples:

```jldoctest; output = false
using MeshArrays
g = GridSpec("CubeSphere",MeshArrays.Dataset("GRID_CS32"),ioPrec=Float32)
isa(g,gcmgrid)

# output

true
```

When the `np` keyword argument is specified then it is used 
to set the number of points in each grid subdomain.

Example:

```jldoctest; output = false
using MeshArrays
g = GridSpec("LatLonCap",np=270)
isa(g,gcmgrid)

# output

true
```
"""
function GridSpec_MITgcm(category="PeriodicDomain", 
    path=tempname(); np=nothing, ID=:unknown, ioPrec=Float64)

if category=="LatLonCap"
    nFaces=5
    grTopo="LatLonCap"
    np === nothing ? np=90 : np
    ioSize=[np np*13]
    facesSize=[(np, np*3), (np, np*3), (np, np), (np*3, np), (np*3, np)]
elseif category=="CubeSphere"
    nFaces=6
    grTopo="CubeSphere"
    np === nothing ? np=32 : np
    ioSize=[np np*nFaces]
    facesSize=[(np, np), (np, np), (np, np), (np, np), (np, np), (np, np)]
elseif category=="PeriodicChannel"
    nFaces=1
    grTopo="PeriodicChannel"
    ioSize=[360 160]
    facesSize=[(360, 160)]
elseif category=="PeriodicDomain"
    nFaces=4
    grTopo="PeriodicDomain"
    ioSize=[80 42]
    facesSize=[(40, 21), (40, 21), (40, 21), (40, 21)]
elseif ID==:unknown
    error("unknown category case")
end

if ID==:unknown
    gcmgrid(path, grTopo, nFaces, facesSize, ioSize, ioPrec, read, write)
elseif ID==:LLC90
    np = 90
    GridSpec("LatLonCap", MeshArrays.Dataset("GRID_LLC90"), np=np, ioPrec=Float64)
elseif ID==:LLC270
    np = 270
    GridSpec("LatLonCap", MeshArrays.Dataset("GRID_LLC270"), np=np, ioPrec=Float32)
elseif ID==:CS32
    np = 32
    GridSpec("CubeSphere", MeshArrays.Dataset("GRID_CS32"), np=np, ioPrec=Float32)
elseif ID==:onedegree
    GridSpec("PeriodicChannel", MeshArrays.Dataset("GRID_LL360"), ioPrec=Float32)
elseif ID==:PeriodicDomain
    GridSpec_MITgcm("PeriodicDomain", ioPrec=ioPrec)
else
    error("unknwown grid")
end

end


