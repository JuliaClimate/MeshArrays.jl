...
begin
	...
	import Shapefile, Dataverse, GeoJSON
...
md"""# Polygons for Ocean Basins

!!! note
	- Read shapefile data set from <https://marineregions.org>
	- Convert to `GeoInterface.Polygon`
	- Simplify using `GeometryOps.simplify`
	- Save to file using `GeoJSON.write`

Relevant websites include:

- [Polygon Data](https://marineregions.org/downloads.php) : here we use `GOaS_v1_20211214/goas_v01.shp`
- [GeometryOps.jl](https://juliageo.org/GeometryOps.jl/dev/tutorials/creating_geometry#save-geometry)
- [GeoInterface.jl](https://juliageo.org/GeoInterface.jl/stable/guides/defaults/#Polygons)
"""

# ╔═╡ 643259db-28db-4349-a0e6-e63466b03efd
md"""## Visualize Ocean Basin Polygons

- plot `#1` : highlight selected ocean basin on map
- plot `#2` : show all ocean basin masks (polygons)
"""

# ╔═╡ 6b5944a4-f8bc-467b-ab69-7dc320240b68
@bind p_select PlutoUI.Select(1:10)
...
function read_polygon(fil; p=1,k=1)
	...
	nk=length(coords)
	println(names[p]*" $(p) of $(length(geoms)) with length = $(nk)")

	pols=Array{GI.Wrappers.Polygon}(undef, nk)
	for kk in 1:nk
		line=GI.LineString(coords[kk][1])
...
# ╔═╡ 1906fc27-73ba-4b1b-ad8c-d98c98debfb9
function write_polygons_to_json(ocean_polygons,path_to_data=tempdir())
	np=length(ocean_polygons.polygons)
	df = DataFrame(geometry=ocean_polygons.polygons)
	df[!,:id] = [:SO,:SAO,:SPO1,:SPO2,:NPO1,:NPO2,:SCS,:IO,:MS,:BS,:NAO,:AO]
	df[!, :name] = ocean_polygons.names

	fn = joinpath(path_to_data,"ocean_basins.json")
	GeoJSON.write(fn, df)
end

# ╔═╡ ae70a18a-7f37-49db-8a16-a5169a2bca74
function simplified_polygons(path_to_data=""; resolution=10000)
	fil = joinpath(path_to_data,"GOaS_v1_20211214","goas_v01.shp")
	polygons=GI.Wrappers.Polygon[]
	names=String[]
	for p in 1:10
		pol,name=read_polygon(fil,p=p)
		
		np=length(GI.coordinates(pol[1])[1])
		nps=resolution*np/500000
		nps=max(round(nps),100)
#		println("np=$(np) nps=$(nps)")
		
		#simplification is important for performance
		push!(polygons,GO.simplify(pol[1],number=nps)) 
		push!(names,name)
		if occursin("South Pacific Ocean",name)||occursin("North Pacific Ocean",name)
			np=length(GI.coordinates(pol[1])[1])
			nps=resolution*np/500000
			nps=max(round(nps),100)
#			println("np=$(np) nps=$(nps)")
			...
			#simplification is important for performance
			push!(polygons,GO.simplify(pol[2],number=nps)) 
			push!(names,name*" 2")
			println("    add : "*name*" 2")
		end
	end
...
# ╔═╡ bd316308-429d-42f6-8a7b-09f6c64a047a
md"""## Read and Simplify Polygons"""

# ╔═╡ 5d4e62f7-f8bb-486e-9017-51eddbaf3618
#path_to_data=joinpath(tempdir(),"ocean_polygons")
path_to_data="."

# ╔═╡ 5d1ad296-d6e5-4269-989d-7c8ac6141f19
ocean_polygons=simplified_polygons(path_to_data,resolution=1000)

# ╔═╡ 2e2b9e9a-fe2a-4dab-8523-b47aa3a620d0
let	
	pol=ocean_polygons.polygons[p_select]
	name=ocean_polygons.names[p_select]
	
	np=length(GI.coordinates(pol)[1])
	fig=Figure(size=(500,300)); ax=Axis(fig[1,1],title=name*" (np=$(np))")
	[lines!(p,color=:grey) for p in ocean_polygons.polygons]
	lines!(pol,color=:red)
	fig
end

# ╔═╡ 63840f39-ddc4-47cc-81de-8b9f3eb4d573
let
	fig=Figure(size=(500,300)); ax=Axis(fig[1,1])
	[lines!(p) for p in ocean_polygons.polygons]
	fig
end

# ╔═╡ 22ef199e-359b-44e3-a6e5-29abd4a0659c
write_polygons_to_json(ocean_polygons,path_to_data)
...
manifest_format = "2.0"
project_hash = "20dd161d40ff424ebd1d5c22bf983f5ee9c3f993"
...
git-tree-sha1 = "c8cb6ddabe30c61b30dcebbce825dd45abc3661e"
